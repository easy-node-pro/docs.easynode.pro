...
<Tabs items={["Fractal cloud-init", "Fractal w/ UFW Settings cloud-init", "Harmony cloud-init", "Harmony w/ UFW Settings cloud-init"]} defaultIndex="0">
  <Tabs.Tab>
    ```text copy
    #cloud-config
  users:
...
## Fractal
If you're going to be installing Fractal you will need to exit your session and start a new one to join the Docker group. Run the command `groups` to verify your account has docker listed as a group.

Pull the runner to your servery by running the following command to also start the installer:
```text copy
cd ~/ && wget https://raw.githubusercontent.com/FindoraNetwork/fractal-toolbox/main/src/bin/fractal.sh && chmod +x fractal.sh && ./fractal.sh --installer
```
