...
<Tabs items={["Check Status and Versions", "Check Docker Logs"]} defaultIndex="0">
  <Tabs.Tab>
    ...
    ```text copy
    python3 ~/fractal-toolbox/src/app.py -s
    ```

    You can also run this remotely from your home terminal via ssh. Customize the command below with your Fractal server username, IP and port. Run the command to pull stats remotely:  

    ```text copy
    ssh [USERNAME]@[IP] -p [PORT] python3 /home/servicefindora/fractal-toolbox/src/app.py -s
    ```
...
After you sucessfully register your wallet and stake your initial Fractal your validator will show up on the [staking dashboard](https://findorascan.io/nodes) along with the [SmartStake site](https://fractal.smartstake.io/).
...
<Tabs items={["Files to Backup", "Termius/winSCP/Filezilla Backup", "rsync/scp Backup"]} defaultIndex="0">
  <Tabs.Tab>
    ...
    - /data/fractal/[network]/tendermint/config folder - This is a copy of your tendermint/config folder which contains priv_validator_key.json
    ```
...
An example of findorad running, congrats! If your container is online and curl still doesn't respond, check your [Firewall](/fractal/new#firewall) instead.
...
- [Fractal Main Dashboard](https://findorascan.io) - Fractal's Official Dashboard & Explorer
- [Fractal SmartStake](https://fractal.smartstake.io) - Stats
- [Fractal Telegram](https://t.me/findoraen) - A telegram chat full of Fractal Validators to ask quesitons. Ask for an invite in the main.
- [Fractal's Official Discord Server](https://mee6.xyz/i/TvaDiyZ9hc) - The central spot for all Fractal news and updates.
