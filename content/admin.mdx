...
Update your firewall to change port 22 to the new port # you chose at this point. Here's a reminder on our [Harmony](/harmony/companion#firewall-setup-notes) & [Fractal](/fractal/new#firewall) firewall notes. Sub your new port for 22 in the guides.
...
<Tabs
  items={["Run Harmony Toolbox", "Show Harmony Stats", "Run Fractal Toolbox"]}
  defaultIndex="0"
>
  <Tabs.Tab>
  ...
  <Tabs.Tab>
    To launch Fractal Toolbox with the latest updates and return you to the user home folder when you exit, make the snippet:  
    
    ```text copy 
    cd ~/ && bash -x ~/fractal.sh
    ```
  </Tabs.Tab>
</Tabs>
...
Docker is required to run Fractal, so with your new server online and logged in to the user you'll be running the programs as, install docker on your Ubuntu 20.04/22.04LTS system with the following command. If you user isn't `servicefindora` update that to be your username in the following code before running it on your server:
...
We have a [repository of Fractal Validator Scripts](https://github.com/easy-node-pro/fractal-validator-scripts) with details on how to use those but consider using the toolbox over these tools going forward. These are still maintained for now.
